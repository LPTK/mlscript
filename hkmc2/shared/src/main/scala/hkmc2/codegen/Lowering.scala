...
class LoweringCtx(
  ...
  */
  def apply(v: Value): Value = v match
    case Value.SimpleRef(l, _) => map.getOrElse(l, v)
    case _ => v
object LoweringCtx:
...
class Lowering()(using Config, TL, Raise, State, Ctx, SymbolPrinter):
  ...
  private def wasmIntrinsicPath(sym: BuiltinSymbol, unary: Bool): Opt[Path] =
    if config.target is CompilationTarget.Wasm then
      ...
      map.get(sym.nme).map(name => State.wasmSymbol.asSimpleRef(N).selN(Tree.Ident(name)))
    else N
  ...
  lazy val unreachableFn =
    Select(State.runtimeSymbol.asSimpleRef(N), Tree.Ident("unreachable"))(S(State.unreachableSymbol))
  
  def unit: Path =
    Select(State.runtimeSymbol.asSimpleRef(N), Tree.Ident("Unit"))(S(State.unitSymbol))
  ...
  def parentConstructor(cls: Term, args: Ls[Term])(using LoweringCtx) = 
    ...
    lowerSuperCtorCall(
      State.builtinOpsMap("super").asSimpleRef(N),
      isMlsFun = true,
      isTailCall = false,
      args.headOption,
      N, // TODO: location?
    ...
    def zipArgs(remainingParamss: Ls[ParamList], remainingArgss: Ls[Term], acc: Ls[Ls[Arg]]): Block =
      ...
      case (Nil, args :: remainingArgss) =>
        ...
        case acc: NELs[Ls[Arg]] =>
          ...
          Assign(tmp, call, lowerRemainingCalls(tmp.asSimpleRef(N), args, remainingArgss, isTailCall, loc)(k))
      case (_ :: _, Nil) =>
        k(Call(fr, acc.reverse.ne_!)(isMlsFun, true, isTailCall).withLoc(loc))
  ...
  def lowerRemainingCalls(base: Path, args: Term, remainingArgss: Ls[Term], isTailCall: Bool, loc: Opt[Loc])
    ...
    lowerArgs(args): as =>
      ...
      case args :: remainingArgss =>
        ...
        Assign(tmp, call,
          lowerRemainingCalls(tmp.asSimpleRef(N), args, remainingArgss, isTailCall, loc)(k))
    ...
    def zipArgs(remainingCtorParamss: Ls[ParamList], remainingArgss: Ls[Term], acc: Ls[Ls[Arg]]): Block =
      ...
      case (Nil, args :: remainingArgss) =>
        ...
        Assign(tmp, buildInstantiate(acc.reverse),
          lowerRemainingCalls(tmp.asSimpleRef(N), args, remainingArgss, isTailCall = false, N)(k))
      case (remainingParamss, Nil) =>
        ...
        def etaExpand(remainingParamss: Ls[ParamList], accArgss: Ls[Ls[Arg]]): Result =
          ...
          case ps :: rest =>
            ...
            val freshArgs = freshSyms.map(s => Arg(N, s.asSimpleRef(N)))
        ...
        lowerArgs(args): as =>
          ...
          case remainingArgss =>
            ...
            Assign(tmp, buildInstantiate(as :: Nil),
              lowerRemainingCalls(tmp.asSimpleRef(N), remainingArgss.head, remainingArgss.tail, isTailCall = false, N)(k))
  ...
  def ref(ref: st.Ref, annots: List[Annot], disamb: Opt[DefinitionSymbol[?]], inStmtPos: Bool)(k: Result => Block)(using LoweringCtx): Block =
    ...
    (sym, disamb) match
      case (sym: (LocalVarSymbol | BuiltinSymbol), _) =>
        k(loweringCtx(sym.asSimpleRef(N).withLocOf(ref)))
  ...
  final def term(t: st, inStmtPos: Bool = false)(k: Result => Block)(using LoweringCtx): Block =
          ...
          term_nonTail(body): r =>
            Assign(
              ...
              Assign(
                isContinue,
                Call(
                  State.builtinOpsMap("===").asSimpleRef(N),
                  (bodyResult.asSimpleRef(N).asArg :: State.runtimeSymbol.asSimpleRef(N).selSN("Continue").asArg :: Nil) ne_:: Nil,
                )(true, false, false),
                Match(
                  isContinue.asSimpleRef(ErasedType.Primitive(PrimitiveType.Bool)),
                  (Case.Lit(Tree.BoolLit(true)) -> Continue(label)) :: Nil,
                  S(Assign(result, bodyResult.asSimpleRef(N), Break(label))),
      ...
      case st.Tup(Nil) =>
        if !sym.nullary then raise:
          ErrorReport(
            ...
            source = Diagnostic.Source.Compilation)
        k(sym.asSimpleRef(N).withLocOf(ref))
      case st.Tup(Fld(FldFlags.benign(), arg, N) :: Nil) =>
        ...
        subTerm(arg): ar =>
          val target = wasmIntrinsicPath(sym, unary = true)
            .getOrElse(sym.asSimpleRef(N).withLocOf(ref))
    ...
    case st.App(f, arg) =>
      ...
      case t if instantiatedResolvedBms.exists(_ is ctx.builtins.js.bitand) =>
        conclude(State.runtimeSymbol.asSimpleRef(N).selN(Tree.Ident("bitand")))
      case t if instantiatedResolvedBms.exists(_ is ctx.builtins.js.bitnot) =>
        conclude(State.runtimeSymbol.asSimpleRef(N).selN(Tree.Ident("bitnot")))
      case t if instantiatedResolvedBms.exists(_ is ctx.builtins.js.bitor) =>
        conclude(State.runtimeSymbol.asSimpleRef(N).selN(Tree.Ident("bitor")))
      case t if instantiatedResolvedBms.exists(_ is ctx.builtins.js.shl) =>
        conclude(State.runtimeSymbol.asSimpleRef(N).selN(Tree.Ident("shl")))
      case t if instantiatedResolvedBms.exists(_ is ctx.builtins.js.try_catch) =>
        conclude(State.runtimeSymbol.asSimpleRef(N).selN(Tree.Ident("try_catch")))
      case t if t.resolvedSym.exists {
        ...
        conclude(State.wasmSymbol.asSimpleRef(N).selN(Tree.Ident(sym.nme)))
        ...
        if !config.effectHandlers.exists(_.debug) then
          return fail:
            ErrorReport(
              ...
              t.toLoc :: Nil,
              source = Diagnostic.Source.Compilation)
        conclude(State.runtimeSymbol.asSimpleRef(N).selSN("raisePrintStackEffect").withLocOf(baseF))
  ...
  def setupTerm(name: Str, args: Ls[Path])(k: Result => Block)(using LoweringCtx): Block =
    k(Instantiate(mut = false, State.termSymbol.asSimpleRef(N).selSN(name), args.map(_.asArg) :: Nil))

  def setupQuotedKeyword(kw: Str): Path =
    State.termSymbol.asSimpleRef(N).selSN("Keyword").selSN(kw)

  def setupSymbol(symbol: Local)(k: Result => Block)(using LoweringCtx): Block =
    k(Instantiate(mut = false, State.termSymbol.asSimpleRef(N).selSN("Symbol"),
  ...
  def quoteSplit(split: Split)(k: Result => Block)(using LoweringCtx): Block = split match
    case Split.Cons(Branch(scrutinee, pattern, continuation), tail) => quote(scrutinee): r1 =>
      ...
      blockBuilder.assign(l1, r1)
        ...
        .chain(b => setupTerm("Branch", (l1 :: l2 :: l3 :: Nil).map(s => s.asSimpleRef(N)))(r4 => Assign(l4, r4, b)))
        .chain(b => quoteSplit(tail)(r5 => Assign(l5, r5, b)))
        .rest(setupTerm("Cons", (l4 :: l5 :: Nil).map(s => s.asSimpleRef(N)))(k))
    case Split.Let(sym, term, tail) => setupSymbol(sym): r1 =>
      loweringCtx.collectScopedSym(sym)
      val l1, l2, l3 = loweringCtx.registerTempSymbol(N)
      blockBuilder.assign(l1, r1)
        .chain(b => setupTerm("Ref", l1.asSimpleRef(N) :: Nil)(r => Assign(sym, r, b)))
        .chain(b => quote(term)(r2 => Assign(l2, r2, b)))
        .chain(b => quoteSplit(tail)(r3 => Assign(l3, r3, b)))
        .rest(setupTerm("Let", (l1 :: l2 :: l3 :: Nil).map(s => s.asSimpleRef(N)))(k))
    case Split.Else(default) => quote(default): r =>
      val l = loweringCtx.registerTempSymbol(N)
      Assign(l, r, setupTerm("Else", l.asSimpleRef(N) :: Nil)(k))
  ...
  lazy val setupFilename: Path =
    ...
    state.importSymbol.asSimpleRef(N).selSN("meta").selSN("url")
  ...
  def quote(t: st)(k: Result => Block)(using LoweringCtx): Block = t match
    ...
    case Resolved(Ref(sym), disamb) =>
      sym match
        ...
        case sym: (LocalVarSymbol | BuiltinSymbol) => k(sym.asSimpleRef(N))
    ...
    case SynthSel(Ref(sym: ModuleOrObjectSymbol), name) => // Local cross-stage references
      setupSymbol(sym): r1 =>
        ...
        Assign(l1, r1, setupTerm("CSRef", l1.asSimpleRef(N) :: setupFilename :: Value.Lit(syntax.Tree.UnitLit(false)) :: Nil)(r2 =>
          Assign(l2, r2, setupTerm("Sel", l2.asSimpleRef(N) :: Value.Lit(syntax.Tree.StrLit(name.name)) :: Nil)(k))
        ))
    case SynthSel(Ref(sym: BlockMemberSymbol), name) => // Multi-file cross-stage references
      if config.qqEnabled then fail:
        ...
        case (S(Loc(_, _, Origin(base, _, _))), S(Loc(_, _, Origin(filename, _, _)))) => setupSymbol(sym): r1 =>
          ...
          Assign(l1, r1, setupTerm("CSRef", l1.asSimpleRef(N) :: setupFilename :: Value.Lit(syntax.Tree.StrLit(relPath)) :: Nil)(r2 =>
            Assign(l2, r2, setupTerm("Sel", l2.asSimpleRef(N) :: Value.Lit(syntax.Tree.StrLit(name.name)) :: Nil)(k))
          ))
        case _ => fail:
          ErrorReport(
      ...
      def rec(ps: Ls[LocalSymbol & NamedSymbol], ds: Ls[Path])(k: Result => Block)(using LoweringCtx): Block = ps match
        case Nil => quote(body): r =>
          ...
          Assign(
            ...
            Assign(l, r, setupTerm("Lam", arr.asSimpleRef(N) :: l.asSimpleRef(N) :: Nil)(k)))
        case sym :: rest =>
          loweringCtx.collectScopedSym(sym)
          setupSymbol(sym): r =>
            val l = loweringCtx.registerTempSymbol(N)
            Assign(l, r, setupTerm("Ref", l.asSimpleRef(N) :: Nil): r1 =>
              Assign(sym, r1, rec(rest, l.asSimpleRef(N) :: ds)(k)))
      ...
      def rec(es: Ls[Elem], xs: Ls[Path])(k: Result => Block): Block = es match
        case Nil =>
          ...
          Assign(
            ...
            setupTerm("Tup", arrSym.asSimpleRef(N) :: Nil): r2 =>
              val l1 = loweringCtx.registerTempSymbol(N)
              val l2 = loweringCtx.registerTempSymbol(N)
              Assign(l1, r1, Assign(l2, r2, setupTerm("App", l1.asSimpleRef(N) :: l2.asSimpleRef(N) :: Nil)(k)))
          )
        case Fld(_, t, _) :: rest => quote(t): r2 =>
          val l = loweringCtx.registerTempSymbol(N)
          Assign(l, r2, rec(rest, l.asSimpleRef(N) :: xs)(k))
    ...
    case Blk(LetDecl(sym, _) :: DefineVar(sym2, rhs) :: Nil, res) => // Let bindings
      ...
      setupSymbol(sym){r1 =>
        ...
        blockBuilder.assign(l1, r1)
          ...
          .chain(b => setupTerm("LetDecl", l1.asSimpleRef(N) :: Nil)(r4 => Assign(l4, r4, b)))
          .chain(b => setupTerm("DefineVar", l1.asSimpleRef(N) :: l2.asSimpleRef(N) :: Nil)(r5 => Assign(l5, r5, b)))
          .assign(arrSym, Tuple(mut = false, (l4 :: l5 :: Nil).map(s => s.asSimpleRef(N).asArg)))
          .rest(setupTerm("Blk", arrSym.asSimpleRef(N) :: l3.asSimpleRef(N) :: Nil)(k))
      }
    case IfLike(_, IfLikeForm.ReturningIf, split) => quoteSplit(split.getExpandedSplit): r =>
      val l = loweringCtx.registerTempSymbol(N)
      Assign(l, r, setupTerm("IfLike", setupQuotedKeyword("If") :: l.asSimpleRef(N) :: Nil)(k))
    ...
    if fsr.isEmpty then
      ...
      Begin(
        ...
        Assign(
          ...
          k((Arg(N, rcdSym.asSimpleRef(N)) :: asr).reverse)))
...
trait LoweringTraceLog(instrument: Bool)(using TL, Raise, State)
  ...
  private val traceLogFn = State.runtimeSymbol.asSimpleRef(N).selSN("TraceLogger").selSN("log")
  private val traceLogIndentFn = State.runtimeSymbol.asSimpleRef(N).selSN("TraceLogger").selSN("indent")
  private val traceLogResetFn = State.runtimeSymbol.asSimpleRef(N).selSN("TraceLogger").selSN("resetIndent")
  ...
  def setupFunctionBody(params: ParamList, bod: Term, name: Option[Str])(using LoweringCtx): Block = inScopedBlock:
    ...
    val psSymArgs = psInspectedSyms.zipWithIndex.foldRight[Ls[Arg]](Arg(N, Value.Lit(Tree.StrLit(")"))) :: Nil):
      case (((s, p), i), acc) => if i == psInspectedSyms.length - 1
        then Arg(N, s.asSimpleRef(ErasedType.Primitive(PrimitiveType.Str))) :: acc
        else Arg(N, s.asSimpleRef(ErasedType.Primitive(PrimitiveType.Str))) :: Arg(N, Value.Lit(Tree.StrLit(", "))) :: acc
    
    val tmp1, tmp2, tmp3 = loweringCtx.registerTempSymbol(N)
    
    assignStmts(psInspectedSyms.map: (pInspectedSym, pSym) =>
      pInspectedSym -> pureCall(inspectFn, Arg(N, pSym.asSimpleRef(N)) :: Nil)
    *) |>:
    assignStmts(
      ...
      tmp1 -> pureCall(traceLogFn, Arg(N, enterMsgSym.asSimpleRef(ErasedType.Primitive(PrimitiveType.Str))) :: Nil),
      prevIndentLvlSym -> pureCall(traceLogIndentFn, Nil)
    ) |>: 
    term(bod)(r =>
    assignStmts(
      resSym -> r,
      resInspectedSym -> pureCall(inspectFn, Arg(N, resSym.asSimpleRef(N)) :: Nil),
      retMsgSym -> pureCall(
        strConcatFn,
        Arg(N, Value.Lit(Tree.StrLit("=> "))) :: Arg(N, resInspectedSym.asSimpleRef(ErasedType.Primitive(PrimitiveType.Str))) :: Nil
      ),
      tmp2 -> pureCall(traceLogResetFn, Arg(N, prevIndentLvlSym.asSimpleRef(N)) :: Nil),
      tmp3 -> pureCall(traceLogFn, Arg(N, retMsgSym.asSimpleRef(ErasedType.Primitive(PrimitiveType.Str))) :: Nil)
    ) |>:
      Ret(resSym.asSimpleRef(N))
    )
